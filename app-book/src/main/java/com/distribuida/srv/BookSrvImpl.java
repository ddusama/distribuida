...
import com.distribuida.dto.bookmapper.BookMp;
...
public class BookSrvImpl implements BookSrv {
    ...
    @Inject
    private BookMp bookMp;

    @Override
    public Book listarUno(int id) throws ExecutionException, InterruptedException {
        ...
        return dbRow.isPresent() ? this.bookMp.read(dbRow.get()) : new Book();
    }

    @Override
    public List<Book> listarTodo() throws ExecutionException, InterruptedException {
        ...
        return execute.collectList().get().stream().map(this.bookMp::read).collect(Collectors.toList());
    }

    @Override
    public long crear(Book book) {
        var rowsChanged = 0L;
        try {
